...
class Module extends Module_Base {
	...
	public function register_controls( $element, $section_id, $args ) {
		...
		$element->start_controls_section(
			...
			[
				'label' => __( 'Schedule Content', 'elementor-controls' ),
		...
		$element->add_control(
			...
			[
				...
				'label_off' => 'No',
				'return_value' => 'yes',
				'description' => __( 'Switch on to schedule the contents of this column|section!.', 'elementor-controls' ),
			]
		);
		...
		$element->add_control(
			...
			[
				...
				'description' => __( 'Set content display schedule start date!', '' ),
			]
		);
		...
		$element->add_control(
			...
			[
				...
				'description' => __( 'Set content display schedule end date!', '' ),
			]
		);
			...
			<script type="text/javascript">
				(function($) {
					$.fn.scheduleContent = function(options) {
						var settings = $.extend({ start: '01/01/2018 00:00:00',end: '12/31/2999 00:00:00'}, options );
						var startDate = new Date(settings.start);var endDate = new Date(settings.end);var now = new Date();
						if((now >= startDate) && (now <= endDate)){$(this).show();} else {$(this).hide();}
					};			
					$('.scheduled-content-<?php echo $element->get_id(); ?>').scheduleContent({start:'<?php echo $start; ?>',end:'<?php echo $end; ?>'});
				})(jQuery);
			</script>	
		<?php	
		}
